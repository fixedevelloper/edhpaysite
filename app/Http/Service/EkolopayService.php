...
class EkolopayService
{
    ...
    public function __construct(LoggerInterface $logger)
    {
        ...
        $this->client = new Client([
            'base_uri' => env('EKOLO_URL'),
        ]);
    }

    function postRequest($data)
    {
        $endpoint = "/api/v1/gateway/purchase-token?api_client=" . env('EKOLOAPI');
        $myuuid = $this->guidv4();
        $product = [
        ...
        $arrayJson = [
            ...
            "amount" => $data['amount'],
            "secret_key" => env('EKOLO_SECRETKEY')
        ];
    ...
    {
        $endpoint = "/api/v1/gateway/check-payment?api_client=" . env('EKOLOAPI');
        $arrayJson = [
            "purchaseToken" => $data,
            "secret_key" => env('EKOLO_SECRETKEY')
        ];
